...
*/
/*
func TestAnupamBasic(t *testing.T) {
	runtime.GOMAXPROCS(4)
...
func TestUnreliable(t *testing.T) {
	...
	fmt.Printf("  ... Passed basic put get\n")
	...
	for iters := 0; iters < 6; iters++ {
		...
		for cli := 0; cli < ncli; cli++ {
			...
			go func(me int) {
				...
				val := myck.Get(key)
				if val != vv {
					t.Fatalf("wrong value-->", val, " expected-->", vv)
				} else {
					fmt.Println("Value matched", vv)
				}
				ok = true
			}(cli)
		}
	...
	fmt.Printf("  ... Passed sequence of puts \n")
